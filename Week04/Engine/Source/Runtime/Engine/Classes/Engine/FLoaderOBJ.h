...
struct FLoaderOBJ
{
    ...
    static bool CreateTextureFromFile(const FWString& Filename)
    {
        ...
        HRESULT hr = FEngineLoop::ResourceManager.LoadTextureFromFile(FEngineLoop::GraphicDevice.Device, FEngineLoop::GraphicDevice.DeviceContext, Filename.c_str());
    ...
    static void ComputeOcclusion(const TArray<FVertexSimple>& InVertices, float& OutOccluder, float& OutOccludee)
    {
        ...
        OutOccluder = sqrtf(MaxRadiusSq);
        OutOccludee = sqrtf(MinRadiusSq);
    }
};
...
public:
    ...
    static bool LoadStaticMeshFromBinary(const FWString& FilePath, OBJ::FStaticMeshRenderData& OutStaticMesh)
    {
        ...
        // Occlusion Sphere
        File.read(reinterpret_cast<char*>(&OutStaticMesh.OccluderRadius), sizeof(float));
        File.read(reinterpret_cast<char*>(&OutStaticMesh.OccludeeRadius), sizeof(float));
        ...
        if (Textures.Num() > 0)
        {
            for (const FWString& Texture : Textures)
            {
                ...
                {
                    FEngineLoop::ResourceManager.LoadTextureFromFile(FEngineLoop::GraphicDevice.Device, FEngineLoop::GraphicDevice.DeviceContext, Texture.c_str());
                }
            }
        }
